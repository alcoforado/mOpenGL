...
private:
    ...
    struct ShapeAllocInfo : IShapeWatcher<VerticeData>,IWatcher<ShapeBatch<VerticeData> >
    {
...
public:
    ...
    virtual void write(IArray<VerticeData> &vertices, IArray<int> &indices, bool bFlush)
    {
    ...
    virtual void ReWriteAll(std::vector<VerticeData> *pvertices,
                            Index StartV,
                            Index SizeV,
...

    {
        ...
        std::vector<VerticeData> &vertices=*pvertices;
...
/* Rewrite the buffer but changing only the minimum it can.
       ...
       while(it!=_v.end())
        {
           ...
           else if (_bRewrite || it->Changed())
           {
            Index lV = it->_shape->nVertices();
            Index lI = it->_shape->nIndices();
            RefArray<VerticeData> vV(*_pVertices,offV,lV);
            RefArray<Index> vI(*_pIndices,offI,lI);
            //If shape expanded rewrite all the shapes from here on
